...
# Create unique output directory with parameters
from datetime import datetime
timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
q_range = f"q{QVALS[0]:.0f}_{QVALS[-1]:.0f}_{len(QVALS)}"
OUTPUT_DIR = REPO_ROOT / "results" / "amplitude_envelope" / "mfdfa" / f"smooth_ms_{SMOOTH_MS}_downsample_{TARGET_SR}_hz_{q_range}_{timestamp}"
...
def process_model(model, path):
    ...
    out_path = OUTPUT_DIR / f"{model}.csv"
